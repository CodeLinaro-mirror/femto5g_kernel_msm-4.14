			  ...
			  struct ethtool_rxfh_param *rxfh)
{
	...
	if (rxfh->key && enetc_si_is_pf(si)) {
		...
		for (i = 0; i < ENETC_RSSHASH_KEY_SIZE / 4; i++) {
			...
			((u32 *)rxfh->key)[i] = enetc_port_rd(hw, ENETC_PRSSK(i));
		}
	}

	/* return RSS table */
	if (rxfh->indir) {
		if (priv->si->get_rss_table)
			err = priv->si->get_rss_table(priv->si, rxfh->indir, priv->si->num_rss);
		else
			err = -EOPNOTSUPP;
	}
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	/* set hash key, if PF */
	if (rxfh->key && enetc_si_is_pf(priv->si))
		enetc_set_rss_key(hw, rxfh->key);

	/* set RSS table */
	if (rxfh->indir) {
		if (si->set_rss_table)
			err = priv->si->set_rss_table(priv->si, rxfh->indir, priv->si->num_rss);
		else
			err = -EOPNOTSUPP;
	}
